...
#### Marshalling
= transform object in memory to bit stream
...
The unique name for a remote reference module is obtained through a combination of registration on the server side and lookup on the client side, facilitated by a naming or directory service. This ensures that clients can correctly and efficiently locate and interact with remote objects in a distributed system.
#### Binding Service
...
NO! Generated by middleware … based on Interface Definition Language
![[Pasted image 20240225213219.png]]
### Putting it all together
![[Pasted image 20240225213235.png]]
#### Steps
##### Client Machine
1. **Step 4.1:** Client calls a remote method (e.g., `remote.add(2, 5)`).
2. **Step 4.2:** Middleware builds the message and opens a socket for communication.
3. **Step 4.3:** The message is forwarded to the local OS and network.
4. **Step 4.4:** The message is sent across the network.
##### Server Machine
5. **Step 4.5:** Local OS forwards the message to the server-side proxy (skeleton).
6. **Step 4.6:** Skeleton accepts the connection and unpacks the message.
7. **Step 4.7:** The local implementation of the `add` method is executed.
8. **Step 4.8:** The result is packed in a reply message.
9. **Step 4.9:** The reply message is forwarded to the local OS.
10. **Step 4.10:** The reply message is sent back across the network.
##### Client Machine
11. **Step 4.11:** The reply message is forwarded to the client-side proxy (stub).
12. **Step 4.12:** Stub unpacks the message.
13. **Step 4.13:** The result is evaluated and returned to the client process.